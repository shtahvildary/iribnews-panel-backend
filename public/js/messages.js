...
(function($) {
  ...
  function replyToMsg(reply, callback) {
    ...
    formData.append(
      "file",
      document.getElementById("inputFile-" + reply.msgId).files[0]
    );
    post("/messages/reply/new", { formData }, function(response) {
  ...
  function cardsAppend(item, userId) {
    ...
    if (
      item.isSeen == 0 &&
      today.getTime() - new Date(item.date).getTime() > 60 * 60 * 24 * 1000
    )
      alarmBorder = true;
    $("#messages-list").append(
      `
        ...
        (item.replys.length > 0
          ...
          : alarmBorder ? "border:5px solid #FFBD33; margin-top:10px; " : "") +
        `">
